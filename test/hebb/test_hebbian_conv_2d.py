...
class TestHebbianConv2d(unittest.TestCase):
    ...
    def test_stride_conversion(self):
        heb_conv = HebbianConv2d(5, 5, 3, stride=3)
        self.assertEqual(heb_conv.stride, (3, 3))

        heb_conv = HebbianConv2d(5, 5, 3, stride=[5, 7])
        self.assertEqual(heb_conv.stride, (5, 7))

        heb_conv = HebbianConv2d(5, 5, 3, stride=(4, 6))
        self.assertEqual(heb_conv.stride, (4, 6))

        try:
            HebbianConv2d(5, 5, 3, stride=[2, 3, 4])
            self.assertTrue(False, "HebbianConvTranpose2d should throw an exception of type Attribute error"
    ...
    def test_dilation_conversion(self):
        heb_conv = HebbianConv2d(5, 5, 3, dilation=3)
        self.assertEqual(heb_conv.dilation, (3, 3))

        heb_conv = HebbianConv2d(5, 5, 3, dilation=[5, 7])
        self.assertEqual(heb_conv.dilation, (5, 7))

        heb_conv = HebbianConv2d(5, 5, 3, dilation=(4, 6))
        self.assertEqual(heb_conv.dilation, (4, 6))

        try:
            HebbianConv2d(5, 5, 3, dilation=[2, 3, 4])
            self.assertTrue(False, "HebbianConvTranpose2d should throw an exception of type Attribute error"
    ...
    def test_padding_conversion(self):
        heb_conv = HebbianConv2d(5, 5, 3, padding=3)
        self.assertEqual(heb_conv.padding, (3, 3))

        heb_conv = HebbianConv2d(5, 5, 3, padding=[5, 7])
        self.assertEqual(heb_conv.padding, (5, 7))

        heb_conv = HebbianConv2d(5, 5, 3, padding=(4, 6))
        self.assertEqual(heb_conv.padding, (4, 6))

        try:
            HebbianConv2d(5, 5, 3, padding=[2, 3, 4])
            self.assertTrue(False, "HebbianConvTranpose2d should throw an exception of type Attribute error"
        ...
        for in_channels in range(1, 3):
            for out_channels in range(1, 3):
                for kernel_size in range(1, 5):
                    heb_conv = HebbianConv2d(in_channels, out_channels, kernel_size)
                ...
                for kernel_size in range(1, 5):
                    ...
                    self.assertTrue(
                        torch.all(
                            ...
                            ).less(TestHebbianConv2d.__epsolon)
                        )
                    )
    ...
    def test_forward_stride(self):
        for in_channels in range(1, 3):
            for out_channels in range(1, 3):
                for kernel_size in range(1, 5):
                    heb_conv = HebbianConv2d(in_channels, out_channels, kernel_size, stride=2,
                                             unit_type=DotUnit(lambda x: x, False))
                    conv = torch.nn.Conv2d(in_channels, out_channels, kernel_size, bias=False, stride=2)
                    conv.weight = heb_conv.weight
                    inp = torch.randn(1, in_channels, 10, 10)
    ...
    def test_forward_dilation(self):
        for in_channels in range(1, 3):
            for out_channels in range(1, 3):
                for kernel_size in range(1, 5):
                    heb_conv = HebbianConv2d(in_channels, out_channels, kernel_size, dilation=2,
                                             unit_type=DotUnit(lambda x: x, False))
                    conv = torch.nn.Conv2d(in_channels, out_channels, kernel_size, bias=False, dilation=2)
                    conv.weight = heb_conv.weight
                    inp = torch.randn(1, in_channels, 10, 10)
