...
default_hebb_params = dict(w_nrm=True, mode='swta', k=0.02, patchwise=True, contrast=1., uniformity=False)
...
class HUNet(nn.Module):
    ...
    def __init__(
        ...
        self.last = HebbConv2d(prev_channels, out_channels, kernel_size=1, **self.hebb_params)
...
class HUNetConvBlock(nn.Module):
    ...
    def __init__(self, in_size, out_size, padding, batch_norm, hebb_params=None):
        ...
        block.append(HebbConv2d(in_size, out_size, kernel_size=3, act=nn.ReLU(), **hebb_params))
        if batch_norm:
            block.append(nn.BatchNorm2d(out_size))
        block.append(nn.ZeroPad2d(padding))
        block.append(HebbConv2d(out_size, out_size, kernel_size=3, act=nn.ReLU(), **hebb_params))
        if batch_norm:
            block.append(nn.BatchNorm2d(out_size))
...
class HUNetUpBlock(nn.Module):
    ...
    def __init__(self, in_size, out_size, up_mode, padding, batch_norm, hebb_params=None):
        ...
        if up_mode == 'upconv':
            self.up = HebbConvTranspose2d(in_size, out_size, kernel_size=2, stride=2, **hebb_params)
        elif up_mode == 'upsample':
            self.up = nn.Sequential(
                nn.Upsample(mode='bilinear', scale_factor=2),
                HebbConv2d(in_size, out_size, kernel_size=1, **hebb_params),
            )
...
def main():
    ...
    model = HUNet(out_channels=1, last_bias=False).to(device)
    print(model)
    output = model(input)
